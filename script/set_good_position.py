...
#ここからmain文
LABEL_LIST = ["OK", "NG"]
IMAGE_DIR = "./Image/" #深さ8bitのオリジナル画像が保存されているディレクトリ
CORRECT_OUTPUT_DIR = "./corrects/" #左側を補正した画像を保存するディレクトリ
ALIGNE_OUTPUT_DIR = "./aligned/"   #位置合わせをした画像を保存するディレクトリ
CLIP_PIXEL = 1500.0 #左から数えてCLIP_PIXELのところまでで区切る
ORIGINAL_IMAGE_WIDTH = 4096.0 #画像の横幅
DEVIDE_RATE = CLIP_PIXEL/ORIGINAL_IMAGE_WIDTH #左端からの割合

for label in LABEL_LIST:
    
    images = []
    images = load_file(IMAGE_DIR + label)

    height,width = images[0].shape[:2]

    for i,img in enumerate(images):
        #まずspecular画像を補正してその時に用いた傾きと切片を返してもらう
        if i%5==4:
            #補正されたcorrect_imgを指定したディレクトリに保存
            ref_img, upper_slope, lower_slope, upper_intercept, lower_intercept, rate, left_img_corner = make_correct_img_leniarly_returnData(img, DEVIDE_RATE)
            cv2.imwrite(CORRECT_OUTPUT_DIR + label + '/correct_image' + str(i).zfill(3) + '.png', ref_img)
            
            #補正された画像の位置合わせを行う
            aligned_ref_img, corner = alignment(ref_img)
            cv2.imwrite(ALIGNE_OUTPUT_DIR + label + '/aligned_image' + str(i).zfill(3) + '.png', aligned_ref_img)
            for j in range(1,5):
                #補正されたcorrect_imgを指定したディレクトリに保存
                left_img, right_img = devide_image(images[i-j], width, DEVIDE_RATE)
                correct_img = homu_with_slope(left_img,right_img,upper_slope,lower_slope,upper_intercept,lower_intercept,left_img_corner)
                fusion_img = hconcat_resize_min([correct_img,right_img])
                cv2.imwrite(CORRECT_OUTPUT_DIR + label + '/correct_image' + str(i-j).zfill(3) + '.png', fusion_img)
                
                #補正された画像の位置合わせを行う
                aligned_fusion_img = convert_img_byUsingCorner(corner, np.float32([[0,height],[0,0],[width,0],[width,height]]),fusion_img)
                cv2.imwrite(ALIGNE_OUTPUT_DIR + label + '/aligned_image' + str(i-j).zfill(3) + '.png', aligned_fusion_img)
'''
#画像からマスク画像を生成するところ

#img = cv2.imread("./corrects/NG/correct_image009.png")
#これが一番きれいだった
img = cv2.imread("./aligned/OK/aligned_image999.png")
img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

correct_img = convert_image2rectangle(img_gray)[0]

result = trim_rect_as_binary(correct_img)
